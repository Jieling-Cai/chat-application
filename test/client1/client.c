...
int main(int argc, char **argv) {
    ...
    /* send the CHAT message line to the server */
    msg2 = (char*)malloc(450);
    bzero(msg2,450);
    construct_Msg(htons(5), username, username2, htonl(strlen("Hello,data received,so far so good")), htonl(21908387), "Hello,data received,so far so good",  msg2);
    int len = strlen("Hello,data received,so far so good");
    n = write(sockfd, msg2, 50+len-9);
    printf("writing %d bytes to server\n",n);
    if (n < 0) 
      error("ERROR writing to socket");
    printf("\nsent first partial chat data\n");
    sleep(2);
    n = write(sockfd, msg2+50+len-9, 4);
    printf("\nsent second partial chat data\n");
    sleep(2);
    n = write(sockfd, msg2+50+len-5, 5);
    printf("\nsent rest partial chat data\n");
